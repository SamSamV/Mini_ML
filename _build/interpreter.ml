...
let eval_prog (p: prog): value =
  ...
  let rec eval (e: expr) (env: value Env.t): value = 
    ...
    | Let(x, e1, e2) -> let v1 = eval e1 env in 
                        ...
                        eval e2 env'
    (*Expresssions Conditionnelles*)
    | If(e1, e2, e3) -> begin match eval e1 env with
                        | VBool n -> if n != false then
                                      eval e2 env
                                    else
                                      eval e3 env
    ...
    (*Fonctions, Applications, Point Fixe*)
    | Fun(x, t, e1) -> let adr = new_ptr() in 
                       Hashtbl.add mem adr (VClos(x,e1,env)); 
                       VPtr(adr)
    | App (f , a) -> (match eval f env with
                      | VPtr(n) -> ( let v=Hashtbl.find mem n in
                        match v with VClos (x, b, env') -> let va = eval a env in
                        eval b (Env.add x va env')
                      | _ -> assert false))
    | Fix(f, x, e1) -> let adr = new_ptr() in
                        Hashtbl.add mem adr(VClos(f,e1,env));
                        VPtr(adr)
    (*Opérateurs de Séquences*)
    (*A completer*)
  ...
  (* Évaluation d'une expression dont la valeur est supposée boolénne 
     qu'on a du créer pour evaluer les bool ce qu'on ne pouvait pas 
     logiquement avec evali*)
  and evalb (e: expr) (env: value Env.t): bool = 
    match eval e env with
    | VBool n -> n
    | _ -> assert false
